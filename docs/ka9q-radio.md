...
The five main **systemctl** commands for controlling **radiod** (or any other
**systemd** service) are:
...
(I'd like to log them, but the *iniparser* library isn't set up to find them.)
...
defaults that would be suitable for the greatest number of situation, so only a few
entries are usually required (specifically **hardware** and **status**).
...
A single multicast group xcan carry many receiver channels, each
...
outbound traffic. The TTL field, which is more accurately a "hop count limit"
...
*ttl defaults to zero*, which means multicast traffic generated by
*radiod* will not even leave the system that generates it. This avoids
...
output ports. "Smart" switches that "snoop" IGMP to confine multicast
traffic to where it is wanted are strongly recommended. ttl = 0 is
fine as long as all applications that read status or received signals
...
commands (see FFTW3.md). FFTW *can* generate this information
...
This document continues in Part 2, where the hardware definition section is described.
