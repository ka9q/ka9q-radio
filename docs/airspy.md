...
km line of sight from a dozen FM broadcast transmitters so I use the
linearity setting.
...
The Airspy HF+ produces complex floating point samples at 8 bytes (64
bits) each. The library callback buffer contains 2,048 samples (16,384
bytes) and this is used when the IP TTL is 0. The default blocksize is
reduced to only 128 samples (1,024 bytes) when the IP TTL > 0 to
still cleanly divide the callback buffer (by 16).

**data-ttl** Default 0. Sets the Internet Protocol
...
raw A/D stream over the Ethernet to another host computer, and then
only if you're *sure* that your network can handle it. Be
especially careful if you have a WiFi base station on your network;
...
multicast traffic to any port that subscribes to it.)
...
set fixed gains for the LNA, mixer and variable gain (baseband)
amplifier stages. The units are supposed to be in decibels but don't
seem well calibrated. Setting any of these values disables the software AGC.
