// $Id: airspyhf.c,v 1.47 2022/04/16 03:20:28 karn Exp karn $
...
int main(int argc,char *argv[]){
  ...
  } else if((Dictionary = iniparser_load("/etc/radio/airspy.conf")) != NULL){
    if(iniparser_find_entry(Dictionary,Name) == 1){
      printf("Using config file /etc/radio/airspy.conf\n");
    } else {
      iniparser_freedict(Dictionary);
      Dictionary = NULL;
    }
  ...
  if(Dictionary == NULL){
    // Search everything under /etc/radio/airspy.conf.d
    char const *subdir = "/etc/radio/airspy.conf.d";
