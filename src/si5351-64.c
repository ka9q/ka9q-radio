...
static void compute_fout_rational(unsigned long long fref_hz,
                                  ...
                                  unsigned R,
                                  unsigned long long *out_num, unsigned long long *out_den);
static uint64_t abs_diff_num_u64(unsigned long long a, unsigned long long b, unsigned long long c, unsigned long long d);
...
bool si5351_solve64(unsigned long long fref_hz, unsigned long long fout_hz, si5351_solution64_t *best)
{
  ...
  best->err_num = (uint64_t)(~(uint64_t)0);
  ...
  for(unsigned R = 1; R <= 128; R <<= 1){
...
#if 0
      uint64_t fpll = (uint64_t)fout_hz * D * R;
#endif
      // Compute PLL ratio X = fpll / fref = (fout/fref) * (D*R)
      // X = (P/Q) * (D*R) = (P*(D*R))/Q
      uint64_t Xn = (uint64_t)P * (D*R);
      uint64_t Xd = (uint64_t)Q;
	...
	continue;
      ...
      uint64_t err = abs_diff_num_u64(fn, fd, fout_hz, 1);
	...
	uint64_t denom = (uint64_t)fout_hz * R;
	uint64_t numer = (uint64_t)fpll_target;

	// Skip if D band doesn't straddle target
	uint64_t Dden = (uint64_t)D * denom;
	uint64_t D1den = (uint64_t)(D+1) * denom;
	if(numer < Dden || numer >= D1den) continue;

	uint64_t frac_num = numer - Dden;   // in [0, denom)
	...
	uint64_t Yn = (uint64_t)D*F + E;
	uint64_t Yd = (uint64_t)F;
	uint64_t Xn = (uint64_t)P * Yn;
	uint64_t Xd = (uint64_t)Q * Yd;
	unsigned long long A = (unsigned long long)(Xn / Xd);
	uint64_t rem = Xn % Xd;
	unsigned long long B=0,C=1;
	if(rem != 0){
  ...
  return best->err_num != (uint64_t)(~(uint64_t)0);
}

// Helpers to get packed register parameters:
void si5351_get_pll_pvals64(const si5351_solution64_t *s, si5351_pvals_t *pll){
  assert(s != NULL);
  assert(pll != NULL);
  *pll = pack_abc(s->A, s->B, s->C);
}
void si5351_get_ms_pvals64(const si5351_solution64_t *s, si5351_pvals_t *ms){
  assert(s != NULL);
  assert(ms != NULL);
                                 ...
                                 unsigned long long *out_num, unsigned long long *out_den)
{
  ...
  while(qq != 0){
    ...
    uint64_t h = (uint64_t)a0 * h_m1 + h_m2;
    uint64_t k = (uint64_t)a0 * k_m1 + k_m2;
	...
	*out_den = last_k;
	return;
      ...
      uint64_t hs = (uint64_t)t * h_m1 + h_m2;
      uint64_t ks = (uint64_t)t * k_m1 + k_m2;
...
// Absolute difference of two rationals a/b and c/d, returned as numerator over common denom in u64
// |a/b - c/d| = |ad - bc| / bd
static uint64_t abs_diff_num_u64(unsigned long long a, unsigned long long b, unsigned long long c, unsigned long long d){
  uint64_t left  = (uint64_t)a * d;
  uint64_t right = (uint64_t)c * b;
...
static si5351_pvals_t pack_abc(unsigned long long a, unsigned long long b, unsigned long long c){
  ...
  // t = floor(128*b/c)
  unsigned long long t = (uint64_t)128u * b / c;
  v.P1 = (unsigned)(128u*a + t - 512u);
  v.P2 = (unsigned)((uint64_t)128u*b - (uint64_t)c*t);
  v.P3 = (unsigned)c;
  return v;
}
...
static bool pll_freq_in_range(unsigned long long fref_hz, unsigned A, unsigned B, unsigned C){
  ...
  uint64_t num = (uint64_t)fref_hz * ((uint64_t)A*C + B);
  uint64_t den = C;

  // 600..900 MHz typical VCO range :contentReference[oaicite:8]{index=8}
  const uint64_t lo = (uint64_t)600000000;
  const uint64_t hi = (uint64_t)900000000;
                                  ...
                                  unsigned long long *out_num, unsigned long long *out_den)
{
  ...
  uint64_t num = (uint64_t)fref_hz * ((uint64_t)A*C + B) * F;
  uint64_t den = (uint64_t)C * ((uint64_t)D*F + E) * R;

  // Reduce to u64 by gcd if possible (fits in u64 first)
